...
# - 레이아웃: [광고] → [요약+본문1] → [상단 이미지 2장] → <hr> → [광고(옵션)] → [중간 이미지 1장] → [본문2]
# - 스타일: 글로벌 1회 + 본문 내 스타일 스니펫 2회 추가(콜아웃/스탯카드/타임라인 등)
# - 광고: .env에서 AD_METHOD=shortcode|raw 로 설정, AD_SHORTCODE 또는 AD_HTML/AD_HTML_FILE 사용
# - 이미지: gpt-image-1 → WebP 압축 → 멀티파트 업로드(ASCII 파일명), 대표 이미지 지정
# - 안전: latin-1 파일명 이슈 해결, BOM 제거, 모더레이션 재시도, excerpt에서 쇼트코드 제거
...
def to_webp(image_bytes: bytes, quality:int=82) -> bytes:
    im = Image.open(io.BytesIO(image_bytes)).convert("RGB")
    buf = io.BytesIO()
    im.save(buf, format="WEBP", quality=quality, method=6)
    return buf.getvalue()

def safe_ascii_filename(title:str, idx:int) -> str:
    stamp = datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S")
    base  = slugify_ascii(title or "image", "image")
    return f"{base}-{stamp}-{idx:02d}.webp"

def wp_upload_media(filename:str, webp_bytes:bytes, alt_text:str) -> dict:
...
def assemble_post(title:str, body1_html:str, body2_html:str, figures_top:list[str], figures_mid:list[str], keyword:str) -> str:
    ad_top = load_ad_block()                     # 첫 광고(필수)
    mod_a, mod_b = rich_modules(title, keyword)  # 본문 내 스타일/모듈(2회)
    parts = []
    parts.append(f"<h1>{title}</h1>")
    parts.append(ad_top)                         # 1) 광고 (미리보기 쇼트코드 권장)
    parts.append(body1_html)                     # 2) 요약+본문1
    parts.append(mod_a)                          #    스타일 스니펫+모듈(1)
    if figures_top: parts.append("\n".join(figures_top))  # 3) 상단 이미지 2장
    parts.append("<hr class='soft'>")
    if AD_INSERT_MIDDLE:
        parts.append(load_ad_block())            # 4) 중간 광고(옵션)
    if figures_mid: parts.append("\n".join(figures_mid))  # 5) 중간 이미지 1장
    parts.append(mod_b)                          #    스타일 스니펫+모듈(2)
    parts.append(body2_html)                     # 6) 본문2
    html = "\n".join(parts)
...
def main():
    ...
    for idx, cap in enumerate(captions, start=1):
        try:
            ...
            webp = to_webp(raw, IMAGE_QUALITY_WEBP)
            fn = safe_ascii_filename(title, idx)
            media = wp_upload_media(fn, webp, alt_text=f"{title} - {cap}")
